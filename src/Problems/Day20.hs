...
import Data.Map (Map, fromList, lookup, empty, insertWith, keys, elems, filter)
...
butt m = Data.Map.filter ((== 4) . length . Prelude.filter (\y -> elem y l) . edges) $ m
    where
        q a n = insertWith (+) a (1 :: Integer) n
        l = keys . Data.Map.filter (== 1) . foldr q empty . concat . Prelude.map edges . elems $ m
